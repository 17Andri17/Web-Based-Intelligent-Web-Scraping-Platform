...
async function main() {
  ...
  const user  = await db.get(`INSERT INTO users (username, password_hash) VALUES ('bk', 'x') RETURNING id`);
  const other = await db.get(`INSERT INTO users (username, password_hash) VALUES ('bkother', 'x') RETURNING id`);
