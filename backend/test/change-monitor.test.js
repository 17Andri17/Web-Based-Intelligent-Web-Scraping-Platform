...
async function main() {
  ...
  const user = await db.get(`INSERT INTO users (username, password_hash, plan) VALUES ('mon', 'x', 'business') RETURNING id`);
  const other = await db.get(`INSERT INTO users (username, password_hash, plan) VALUES ('monother', 'x', 'business') RETURNING id`);
