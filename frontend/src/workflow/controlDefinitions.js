...
export const controlDefinitions = {
  ...
  [CONTROL_TYPES.FOR_EACH_ROW]: {
    ...
    description: 'For every row of a list, run steps (optionally on its link\'s page) and add the results back onto the row.',
    color:       '#2dd4bf',   // teal (matches enrich)
    bgColor:     'rgba(45, 212, 191, 0.08)',
    icon:        '⊞',
    branches: [
      { key: 'body', label: 'Steps to run for each row',
        emptyLabel: 'Add steps to run per row. Name each extraction — those names become new columns. Reach the row with {{row.column}}.' },
    ],
    params: {
      source: {
      ...
      openUrlField: {
        type: 'columnSelect',
        label: 'Open link column  (optional)',
      ...
      itemVar: {
        type: 'string',
        label: 'Row variable  (use as {{row.column}})',
        default: 'row',
        placeholder: 'row',
      },
      indexVar: {
