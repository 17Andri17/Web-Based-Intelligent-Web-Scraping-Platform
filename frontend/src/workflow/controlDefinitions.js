...
export const controlDefinitions = {
  ...
  [CONTROL_TYPES.FOR_EACH_ROW]: {
    label:       'For Each Row (Enrich a List)',
    description: 'Take a list you already scraped, and for every row run some steps and add the results back onto that row. Optionally open each row\'s link first to scrape its detail page. This is the built-in "list → details" pattern — no separate saved workflow needed.',
    color:       '#2dd4bf',   // teal (matches enrich)
    bgColor:     'rgba(45, 212, 191, 0.08)',
    icon:        '⊞',
    branches: [
      { key: 'body', label: 'Steps to run for each row',
        emptyLabel: 'Add the steps to run for every row. Name each extraction — those names become the new columns. Use {{row.column}} to reach the current row\'s values.' },
    ],
    params: {
      source: {
        ...
        help: 'Pick a list you scraped earlier (an Extract List step). The steps below run once per row.',
      },
      openUrlField: {
        type: 'columnSelect',
        label: '2. Open a link from this column? (optional)',
        placeholder: 'link',
        columnOf: 'source',
        optional: true,
        help: 'Leave as “none” to run the steps on the current page. Pick a column to open each row\'s detail page first.',
      },
      baseUrl: {
        type: 'string',
      ...
      itemVar: {
        type: 'string',
        label: 'Name for the current row (use it in steps as {{row.column}})',
        default: 'row',
        placeholder: 'row',
      },
      indexVar: {
        type: 'string',
        label: 'Name for the row number (optional)',
        default: 'index',
        placeholder: 'index',
      },
      timeout: {
      ...
      mergeStrategy: {
        type: 'select',
        label: '3. How should the results be combined with each row?',
        default: 'flat',
        options: [
          { label: 'Add the results as new columns  (recommended)',     value: 'flat' },
          { label: 'Add as new columns, but prefix their names',        value: 'prefix' },
          { label: 'Keep the results together under one column',        value: 'nest' },
      ...
      detailField: {
        type: 'string',
        label: 'Name of the column to hold the results',
        default: 'detail',
        placeholder: 'detail',
      ...
      explodeField: {
        type: 'string',
        label: 'Which result list should become one-row-per-item?',
        placeholder: 'reviews  — the name of a list your steps produce',
        help: 'Leave blank to auto-pick the first list your steps produce.',
        showIf: { mergeStrategy: ['explode'] },
      },
      outputVar: {
        type: 'string',
        label: 'Save the enriched list under (optional)',
